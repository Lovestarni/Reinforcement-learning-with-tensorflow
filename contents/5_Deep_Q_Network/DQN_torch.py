...
class DeepQNetwork:
    def __init__(self,
        ...
        if self.output_graph:
            ...
            self.writer.add_graph(self.eval_net, torch.FloatTensor([[2,2]]))

    def choose_action(self, obeservation):
        obeservation = torch.unsqueeze(torch.FloatTensor(obeservation), 0)
        # input only one sample
        if np.random.uniform() < self.epsilon:  # greedy
            actions_value = self.eval_net.forward(obeservation)
